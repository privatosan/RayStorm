...
char *app_errors[] =
{
	...
	"No picture renderd",
	"Can't open screen",
	"Can't open raystorm.library V20",
...
#ifdef __AMIGA__
	RayStormBase =  OpenLibrary((unsigned char*)"raystorm.library",2L);
	if (!RayStormBase)
...
#else
	return rsiSetScreen(CTXT, atoi((char *)arg[0]), atoi((char *)arg[0]));
#endif
}
...
ULONG imtexture(ULONG *arg)
{
	...
	err = rsiAddTexture(CTXT, &texture,(char*)arg[0],surf->item);
	if(err)
		return err;
