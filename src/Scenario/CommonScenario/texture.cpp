...
rsiResult HYPER_TEXTURE::ToRSI(rsiCONTEXT *rc, void **texture, void *surf)
{
	...
	do
	{
		err = PPC_STUB(rsiAddHyperTexture)(CTXT, texture, GetName(), surf);
		if(err)
		{
			// can't load texture -> ask user for other filename
			PPC_STUB(rsiGetErrorMsg)(CTXT, buffer, err);
			...
			{
				strcpy(buffer, GetName());
				retry = utility.FileRequ(buffer, FILEREQU_HTEXTURE, FILEREQU_INITIALFILEDIR);
				if(retry)
					SetName(buffer);
				else
					return err;
...
TEXTURE_OBJECT::TEXTURE_OBJECT()
{
	...
	SetVector(&pos, 0.02f, 0.02f, 0.02f);
	texture = NULL;
}
