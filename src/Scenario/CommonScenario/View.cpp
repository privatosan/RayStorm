...
void VIEW::SetViewing(CAMERA *camera)
{
	...
	if ((viewmode == VIEW_PERSP || viewmode == VIEW_CAMERA) && camera)
	{
		...
		NPC_DC.m[2*4+2] = (float)-height;   // y scale
	...
	else
	{
		...
		switch (viewmode)
		{
			case VIEW_FRONT:
				...
				SetVector(&v, (float)((width>>1) + left), (float)((height>>1) + top), 0.f);
			...
			case VIEW_RIGHT:
				...
				SetVector(&v, 0.f, (float)((height>>1) + top), (float)((width>>1) + left));
				VRC_NPC.SetTMatrix(&v);
				MC_DC.MultMat(&NPC_WC, &VRC_NPC);
				MC_DC.m[1] = MC_DC.m[3];
				MC_DC.m[5] = MC_DC.m[7];
			...
			case VIEW_TOP:
				...
				SetVector(&v, (float)((width>>1) + left), 0.f, (float)((height>>1) + top));
				VRC_NPC.SetTMatrix(&v);
				MC_DC.MultMat(&NPC_WC, &VRC_NPC);
				MC_DC.m[2] = MC_DC.m[3];
				MC_DC.m[6] = MC_DC.m[7];
