...
{
	BOOL objInit();
	char *objRead(rsiCONTEXT*, char*, OBJLINK *, const VECTOR*, const VECTOR*, const VECTOR*, const VECTOR*, const VECTOR*, ACTOR *, SURFACE *, ULONG*, void (*)(rsiCONTEXT*, float));
	void objCleanup();
}*/
...
#ifndef PROGRESSWINDOW_CLASS_H
#include "progresswindow_class.h"
#endif
...
typedef char *(*READWRITEPROC)(rsiCONTEXT*, char *, OBJLINK *, const VECTOR *, const VECTOR *, const VECTOR *, const VECTOR *, const VECTOR *, ACTOR *, SURFACE *, ULONG *, void (*)(rsiCONTEXT*, float));
	...
	if((display->editmode & EDIT_ACTIVE) && (display->editmode & EDIT_OBJECT))
	{
		if(selected)
		{
			...
			if(!out)
			{
				...
				{
					PPC_STUB(glPushMatrix)();
					PPC_STUB(glLoadName)((GLuint)this);
					OGLLoadMatrix(stack->GetMatrix());
					Draw(display,stack);
					PPC_STUB(glPopMatrix)();
				}
...
OBJECT *TestSelected(DISPLAY *display,WORD x,WORD y,BOOL multi)
{
	...
	if(root)
	{
		...
		if(display->display == DISPLAY_SOLID)
		{
			ULONG select_buffer[OPENGL_SELECT_BUFFER_SIZE], *cur_sel;
			ULONG hits;
			OBJECT *down;

			// init select mode
			OpenGLInitSelect(x,y,display->view->left,display->view->right,display->view->top,display->view->bottom,select_buffer);
			// redraw view
			sciRedrawCurrentView();
			hits = OpenGLQuitSelect();

			cur_sel = select_buffer;
			while(hits--)
			{
				if(cur_sel[0] && cur_sel[3])
				{
					selobj = (OBJECT*)cur_sel[3];
					// deselect all other objects if multiselect is off
					if(!multi)
						root->DeselectList(selobj, display->editmode & EDIT_MESH);

					if(!selobj->selected)
						selobj->Select();

					down = (OBJECT*)selobj->GoDown();
					// select all child objects
					if(down)
						down->SelectList();
...
void OBJECT::SetActiveObject(DISPLAY *disp, MATRIX_STACK *stack, BOOL add)
{
	...
	if(add)
	{
		...
		if(disp->localalign)
		{
			...
			orient_y = oy;
			orient_z = oz;
			align.x += disp->align.x;
			align.y += disp->align.y;
			align.z += disp->align.z;
		}
		else
		{
			// rotation of an object in the world coordinate system
			// Formula:
			//
			//             -1
			// mo = mr * mw
...
/*************
 * DESCRIPTION:   callback function for progress meter
 * INPUT:         rc       pointer to context (in this case window handle to progress window)
 *                progress progress value (0.f - 1.f)
 * OUTPUT:        error string or NULL
 *************/
void SetProgress(rsiCONTEXT *rc, float progress)
{
#ifdef __AMIGA__
	Object *win = (Object*)rc;

	SetAttrs(win, MUIA_Progress_Percent, (ULONG)(progress*65535.f), TAG_DONE);
#endif // __AMIGA__
}
...
char *LoadObject(char* filename, MESH *root)
{
	...
	VECTOR ox = {1.f,0.f,0.f};
	VECTOR oy = {0.f,1.f,0.f};
	VECTOR oz = {0.f,0.f,1.f};
	UNDO_CREATE *undo;
	ULONG version = 0;
	...
	if(identok)
	{
		...
		// open display window
		win = (Object*)NewObject(CL_ProgressWindow->mcc_Class,NULL,TAG_DONE);
		if(win)
		{
...
#ifdef __MIXEDBINARY__
		errstr = ppc_objRead(win, filename, &link, &pos, &ox, &oy, &oz, &scale, NULL, NULL, &version);
#else
		errstr = objRead(win, filename, &link, &pos, &ox, &oy, &oz, &scale, NULL, NULL, &version, SetProgress);
#endif // __MIXEDBINARY__
...
#else // __AMIGA__
		errstr = objRead(NULL, filename, &link, &pos, &ox, &oy, &oz, &scale, NULL, NULL, &version, SetProgress);
#endif // __AMIGA__
