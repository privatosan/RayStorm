...
void DoMouseMove(DISPLAY *display, int x, int y, MOUSEDATA *mouse_data)
{
	...
	mdx = (x - mouse_data->oldmousex)*SCALE_MOUSE_DELTA;
	mdy = (y - mouse_data->oldmousey)*SCALE_MOUSE_DELTA;
		...
		case EDIT_ROTATE:
			switch (view->viewmode)
			{
				...
				case VIEW_FRONT:
					if(display->editmode & EDIT_OBJECT)
					{
						SetObjectAlign(display, mouse_data, -mouse_data->dz, -mouse_data->dx, mouse_data->dy);
					}
					break;
				case VIEW_RIGHT:
					if(display->editmode & EDIT_OBJECT)
					{
						SetObjectAlign(display, mouse_data, mouse_data->dy, -mouse_data->dx, mouse_data->dz);
					}
					break;
				case VIEW_TOP:
					if(display->editmode & EDIT_OBJECT)
					{
						SetObjectAlign(display, mouse_data, -mouse_data->dx, mouse_data->dz, mouse_data->dy);
					}
					break;
			}
			break;
		case EDIT_MOVE:
			switch(view->viewmode)
			{
				...
				case VIEW_CAMERA:
					if(display->editmode & EDIT_OBJECT)
					{
						SetVector(&v, mouse_data->dx, mouse_data->dy, mouse_data->dz);
						SetVector(&u,
