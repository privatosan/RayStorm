...
void OpenGLDrawSphere(float radius)
{
	...
	for(i = 0; i <= 360; i += OGL_SPHERE_DIVS)
	{
		...
		glVertex3f(u.x*radius, f2, u.z*radius);
	}
	glEnd();
	...
	for(j=OGL_SPHERE_DIVS; j<180-OGL_SPHERE_DIVS; j+=OGL_SPHERE_DIVS)
	{
		...
		for(i = 0; i <= 360; i += OGL_SPHERE_DIVS)
		{
			...
			v.z *= f1;
			glNormal3fv((GLfloat*)&u);
			glVertex3f(u.x*radius, u.y*radius, u.z*radius);
...
void OpenGLSetupCamera(DISPLAY *display, COLOR *clearcolor, VECTOR *pos, VECTOR *look, VECTOR *up, float near, float far)
{
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	gluPerspective(display->camera->vfov*2.f*INV_PI_180, -(float)display->view->width/(float)display->view->height, near, far);
	gluLookAt(
		pos->x, pos->y, pos->z,
...
void OpenGLSetupPerspective(DISPLAY *display, COLOR *clearcolor, VECTOR *pos, VECTOR *look, VECTOR *up, float near, float far)
{
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	gluPerspective((float)global.yres/(float)global.xres*INV_PI_180, -(float)display->view->width/(float)display->view->height, near, far);
	gluLookAt(
