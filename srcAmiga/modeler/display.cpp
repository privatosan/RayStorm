 ...
 * INPUT:         near     near view distance
 *                far      far view distance
 * OUTPUT:        see INPUT near and far
 *************/
void DISPLAY::GetViewRange(float *near, float *far)
{
	float dx,dy,dz;
...
void DISPLAY::SetupOpenGL()
{
	...
	switch(view->viewmode)
	{
		case VIEW_CAMERA:
			...
			m.MultVectMat(&up);
			OpenGLSetupCamera(this, &clearcolor, &pos, &look, &up, near, far);
			break;
		case VIEW_PERSP:
			GetViewRange(&near, &far);
			...
			SetVector(&up, 0.f, 1.f, 0.f);
			m.MultVectMat(&up);
			OpenGLSetupPerspective(this, &clearcolor, &pos, &look, &up, near, far);
			break;
		case VIEW_RIGHT:
