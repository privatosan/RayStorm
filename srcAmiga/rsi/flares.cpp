...
void FLARE::Draw(SMALL_COLOR *line, int fx, int x, int y)
{
...
#else
		alpha = atan2(y, x) + tilt;
#endif
		if (alpha < 0.f)
			alpha += 2.f*PI;

		alpha = fmod(alpha, 2.f*PI/edges);
		hr = sinphi/sin(PI - phi - alpha);
	...
	switch (func)
	{
		...
		case FLARE_EXP1:
					...
					intensity.r =
					intensity.g =
					intensity.b = exp(-100.f*f*f);
					break;
		case FLARE_EXP2:
					f = (r - hr)/hr;
					intensity.r =
					intensity.g =
					intensity.b = exp(-10.f*f*f);
					break;
		case FLARE_EXP3:
					f = (r - hr)/hr;
					intensity.r =
					intensity.g =
					intensity.b = exp(-2.f*f*f);
					break;
		case FLARE_RADIAL:
					intensity.r =
					intensity.g =
		...
		case FLARE_RAINBOW:
					f = ((r - hr*.9f)/hr);
					intensity.r = exp(-40.f*f*f);
					f = ((r - hr*.8f)/hr);
					intensity.g = exp(-40.f*f*f);
					f = ((r - hr*.7f)/hr);
					intensity.b = exp(-40.f*f*f);
					break;
	}
