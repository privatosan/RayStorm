			...
			if(light->DoLight(&light_to_hit, &newcolor))
			{
				if((light->flags & LIGHT_SHADOW) && !(rc->world.flags & WORLD_QUICK) && !(surf->flags & BRIGHT))
				{
					if(light->shadowmap)
					{
						...
						intens = light->InterpolateShadowMap(u,v, dist_light_hit-EPSILON);
						newcolor.r *= intens;
						newcolor.g *= intens;
						newcolor.b *= intens;
						shadow = NULL;
					...
					else
					{
						...
						do
						{
							...
							if(shadow)
							{
								...
								VecAddScaled(&newray.start, newray.lambda+5e-4, &newray.dir, &newray.start);
			 ...
			 lightcolor.b > EPSILON)
		{
				...
				if (surf->translucency > EPSILON)
				{
					...
					if (surf->transphong > EPSILON)
					{
						...
						if(cosalpha > EPSILON)
						{
							...
							// appropriate power.
							intens = pow(cosalpha, surf->transphong);
			...
			else
			{
				...
				if(surf->refphong > EPSILON)
				{
					...
					if(cosalpha > EPSILON)
					{
						intens = pow(cosalpha, surf->refphong);
			...
			if (!total_int_refl)
			{
				...
				{ // ray hits!
					...
					if (surf->foglength > EPSILON)
					{
						...
						intens = intens < -10.f ? 0.f : exp(intens);
				...
				if(light->shadowmap)
				{
					...
					for(i=0; i<light->shadowmap_size>>1; i++)
					{
						for(j=0; j<light->shadowmap_size; j++)
						{
							...
							SetVector(&ray.dir, su*cos(v), u, su*sin(v));
