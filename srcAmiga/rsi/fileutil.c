...
/*************
 * FUNCTION:    AddPart
 * DESCRIPTION:   <file> is a file path and <name> is a name
...
int AddFileExt(char *file, char *ext, int maxlen)
{
	...
	if (strlen(file) + 1 > maxlen)
		return 0;
		
	strcat(file, ".");

	cnt = strlen(ext) + 1;
	if (strlen(file) + cnt > maxlen)
 ...
 *                origfile    name of file to test
 *                buffer  buffer for the the result (at least 256 chars long)
 * OUTPUT:        TRUE if found else FALSE
 **********/
BOOL ExpandPath(char *paths, char *origfile, char *buffer)
{
	FILE *test;
	char *pathptr = paths, *help, *file;

	file = new char[strlen(origfile)+1];
	if(!file)
		return FALSE;

	// convert the slashes (backslashes)
	strcpy(file, origfile);
	help = file;
	while(help)
	{
#ifdef __AMIGA__
