...
BOOL SOR::Compute(VECT2D *p)
{
	...
	for(i=0; i<number; i++)
	{
		...
		spline[i].r2 = MAX(MAX(x[0], x[1]), MAX(x[2], x[3]));
	...
	cap_radius = ((spline[number-1].a * w + spline[number-1].b) * w + spline[number-1].c) * w + spline[number-1].d;
	if(cap_radius < 0.f)
		cap_radius = 0.f;

	// Get base radius
	w = spline[0].h1;
	base_radius = ((spline[0].a * w + spline[0].b) * w + spline[0].c) * w + spline[0].d;
	if(base_radius < 0.f)
		base_radius = 0.f;
