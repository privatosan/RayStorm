...
static int Raytrace(RSICONTEXT *rc, const TIME *cur_time)
{
	...
	if (keyDecodeKeyfile1)
		rc->world.validkeyfile = TRUE;//keyDecodeKeyfile1();
#endif
...
*              rsiTWorldHyperStep      float    step value for hypertextures
*              ----------------------------------------------------
* INPUT:       rc    render context
* OUTPUT:      rsiResult
*************/
rsiResult rsiSetWorld(CTXT, ...)
{
	...
	static PARAM para[] =
	{
		...
		{rsiTWorldHyperStep, rsiTypeFloat, NULL},
		{rsiTDone, 0, 0}
	};
	...
	para[8].data = (void*)&rc->world.step;
...
*              rsiTTextureParams    void*    raystorm texture parameters
*              ----------------------------------------------------
...
rsiResult rsiSetTexture(CTXT, TEXTURE *texture, ...)
{
	...
	static PARAM para[] =
	{
		...
		{rsiTTextureParams, rsiTypePointer, NULL},
		{rsiTDone, 0, 0}
	};
	...
	if(params)
	{
		if(((RAYSTORM_TEXTURE*)texture)->data)
			memcpy(((RAYSTORM_TEXTURE*)texture)->data, params, ((RAYSTORM_TEXTURE*)texture)->tinfo->datasize);
	}
...
* INPUT:       texturep    pointer pointer which receives the pointer to
*                          the created texture
*              type        type of hyper texture
*              surf        surface to apply the texture to
* OUTPUT:      rsiResult
*************/
rsiResult rsiAddHyperTexture(CTXT, HYPERTEXTURE **texturep, int type, SURFACE *surf)
{
	if(!surf)
		return ERR_SURFACE;
	...
	switch(type)
	{
		case rsiFHyperExplosion:
			*texturep = new EXPLOSION;
			break;
	}
	if(!*texturep)
		return ERR_MEM;
...
/************
* DESCRIPTION: set hyper texture properties
*              tag names            type     meaning
*              ----------------------------------------------------
*              rsiTHyperPos         VECTOR   position
*              rsiTHyperAlign       VECTOR   alignment
*              rsiTHyperSize        VECTOR   size
*              rsiTHyperOrientX     VECTOR   x orientation
*              rsiTHyperOrientY     VECTOR   y orientation
*              rsiTHyperOrientZ     VECTOR   z orientation
*              rsiTHyperActor       ACTOR    actor
*              rsiTHyperP1          float    parameter 1
*              rsiTHyperP2          float    parameter 2
*              rsiTHyperP3          float    parameter 3
*              ----------------------------------------------------
* INPUT:       texture  pointer to hyper texture
* OUTPUT:      rsiResult
*************/
rsiResult rsiSetHyperTexture(CTXT, HYPERTEXTURE *texture, ...)
{
	VECTOR *pos=NULL,*align=NULL,*size=NULL,
		*orient_x=NULL,*orient_y=NULL,*orient_z=NULL;
	va_list list;
	...
	static PARAM para[] =
	{
		{rsiTHyperPos, rsiTypePointer, NULL},
		{rsiTHyperAlign, rsiTypePointer, NULL},
		{rsiTHyperSize, rsiTypePointer, NULL},
		{rsiTHyperOrientX, rsiTypePointer, NULL},
		{rsiTHyperOrientY, rsiTypePointer, NULL},
		{rsiTHyperOrientZ, rsiTypePointer, NULL},
		{rsiTHyperActor, rsiTypePointer, NULL},
		{rsiTHyperP1, rsiTypeFloat, NULL},
		{rsiTHyperP2, rsiTypeFloat, NULL},
		{rsiTHyperP3, rsiTypeFloat, NULL},
		{rsiTDone, 0, 0}
	};

	para[0].data = (void*)&pos;
	para[1].data = (void*)&align;
	para[2].data = (void*)&size;
	para[3].data = (void*)&orient_x;
	para[4].data = (void*)&orient_y;
	para[5].data = (void*)&orient_z;
	para[6].data = (void*)&texture->actor;
	para[7].data = (void*)&texture->p1;
	para[8].data = (void*)&texture->p2;
	para[9].data = (void*)&texture->p3;
