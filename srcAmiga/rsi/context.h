...
class RSICONTEXT
{
	...
	public:
		...
		void Insert(ACTOR *actor) { actor->LIST::Insert((LIST**)&actor_root); }
		void Insert(IMAGE *image) { image->LIST::Insert((LIST**)&image_root); }
		void Insert(LIGHT *light) { light->LIST::Insert((LIST**)&light_root); }
		void Insert(OBJECT *obj)
		{
			if(obj->flags & OBJECT_UNLIMITED)
				obj->LIST::Insert((LIST**)&obj_unlimited_root);
			else
				obj->LIST::Insert((LIST**)&obj_root);
		}
		void Insert(SURFACE *surface) { surface->LIST::Insert((LIST**)&surf_root); }
		void Insert(IM_TEXT *texture) { texture->LIST::Insert((LIST**)&text_root); }
};
